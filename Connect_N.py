...
def make_move(board, max_rows, max_cols, col, player):
    ...
    -1
    """
    if col>=0 and col<max_cols and type(col)==int:
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    False
    """
    return (player*num_connect) in ''.join([get_piece(board, row, element) for element in range(max_cols)])
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    """
    counter = 0
    for element in range(max_rows):
        if get_piece(board, element, col)==player:
            counter+=1
        else:
            counter=0
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    return diagonal_win or vertical_win or horizontal_win
